...
picrq=function(U,V,delta,x,tau,estimation=NULL,var.estimation=NULL,wttype="Param",hlimit=NULL,id=NULL,index=1,maxit=100,tol=1e-3){
  ...
  while (i<max.iter & eps >= tol ) {
    ...
    if(is.null(estimation)){
      ...
      if(exactnum< (0.2*n)){
        new_beta = c(old_beta) - solve(Amat)%*%Efunc2(U=U,V=V,x=x,delta=delta,tau=tau,ww=ww,eta=eta,cluster=cluster,beta = old_beta)/(n)
      }
      else{
      ...
      new_beta = c(old_beta) - solve(Amat)%*%DREfunc(U=U,V=V,x=x,delta=delta,tau=tau,ww=ww,wr=wr,eta=eta,cluster=cluster,beta = old_beta, Sigma = old_Sigma)/(n)
    }
    if(var.estimation=="IS"){
      ...
      new_beta = BB::dfsane(par=beta,fn=Efunc2,U=U,V=V,x=x,delta=delta,tau=tau,ww=ww,eta=eta,cluster=cluster,control=list(trace=FALSE))$par
