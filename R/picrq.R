...
#' picrq(U=log(U),V=log(V),delta=delta,x=x,tau=tau,estimation = "DR",var.estimation = "IS")
#' 
#' 
#' # Data example
#' library(PICBayes)
...
#'picrq(U=U,V=V,delta,x=x,tau=tau,estimation = "DR",var.estimation = "IS")
...
picrq=function(U,V,delta,x,tau,estimation=NULL,var.estimation=NULL,wttype="KM",hlimit=NULL,id=NULL,index=1,maxit=100,tol=1e-3){
  ...
  Gfunc2 = function(U,V,x,delta,tau,ww,eta,cluster,beta,Sigma,B=100) {
    n=length(U);
    library(MASS)
    Shat = t(replicate(B,{
      id = sample(n,n,replace = TRUE)
      Efunc(U=U[id],V=V[id],x=x[id,],delta=delta[id],tau=tau,ww=ww[id],eta=eta[id],cluster=cluster,beta = beta, Sigma = Sigma)
    }))
    Var = cov(Shat*n)
    solve(Var)
  }
  
  Gfunc3= function(U,V,x,delta,tau,ww,eta,id,cluster,beta,Sigma,B=100) {
    n=length(U)
    # tabid=as.vector(table(id))
    # idx = as.vector(unlist(lapply(tabid, function(x) sample(x=x,size=x,replace = TRUE))))
    df=data.frame(num=1:n, id=id, U=U, V=V, ww=ww, x=x, eta=eta, delta=delta)
    ...
    for (i in 1:len) {
      ...
      Shat = t(replicate(B,{
        ...
        Efunc(U=df2$U[idx],V=df2$V[idx],x=newx[idx,],delta=df2$delta[idx],tau=tau,ww=df2$ww[idx],eta=df2$eta[idx],cluster=cluster,beta = beta, Sigma = Sigma)
      }))
  ...
  while (i<max.iter & eps >= tol ) {
    Amat = Afunc(U=U,V=V,x=x,delta=delta,tau=tau,ww=ww,eta=eta,cluster=cluster,beta = old_beta, Sigma = old_Sigma)
    if(is.null(estimation)){
      new_beta = c(old_beta) - solve(Amat)%*%Efunc(U=U,V=V,x=x,delta=delta,tau=tau,ww=ww,eta=eta,cluster=cluster,beta = old_beta, Sigma = old_Sigma)/(n)
    }
    else if(estimation=="DR"){
      wr=Vwtpicft(U=U,V=V,delta=delta)
      new_beta = c(old_beta) - solve(Amat)%*%DREfunc(U=U,V=V,x=x,delta=delta,tau=tau,wr=wr,ww=ww,eta=eta,cluster=cluster,beta = old_beta, Sigma = old_Sigma)/(n)
    }
    if(var.estimation=="IS"){
      Gamma = Gfunc(U=U,V=V,x=x,delta=delta,tau=tau,ww=ww,eta=eta,cluster=cluster,beta = old_beta, Sigma = old_Sigma)
      ...
      new_Sigma = Gfunc2(U=U,V=V,x=x,delta=delta,tau=tau,ww=ww,eta=eta,cluster=cluster,beta = old_beta, Sigma = old_Sigma)
    }
    else if(var.estimation=="Bootstrap" & is.null(id)==FALSE){
      # new_beta = BB::dfsane(par=beta,fn=Efunc2,L=L,R=R,x=x,delta=delta,tau=tau,ww=ww,eta=eta,cluster=cluster,control=list(trace=FALSE))$par
      new_Sigma = Gfunc3(U=U,V=V,x=x,delta=delta,tau=tau,ww=ww,eta=eta,id=id,cluster=cluster,beta = old_beta, Sigma = old_Sigma)
    }
    if (det(new_Sigma) <= 0) {
