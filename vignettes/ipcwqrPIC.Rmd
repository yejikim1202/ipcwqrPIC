...
devtools::install_github(repo="YejiStat/ipcwqrPIC")
```
...
picrq(L=L,R=R,delta=delta,x=x,tau=tau,var.estimation = "IS")
```
...
n = 200
tau=0.1
x1=runif(n,-0.1,1.5); x2=rbinom(n,1,0.6)
T = 1+x1+x2+(rnorm(n)-qnorm(tau))*(1.1-0.05*x2)
x = cbind(x1,x2)
...
tau=0.3
ipcwqrPIC::picrq(L=U,R=V,delta=delta,x=x,tau=tau,wttype="param",var.estimation="IS")
ipcwqrPIC::picrq(L=U,R=V,delta=delta,x=x,tau=tau,wttype="param",var.estimation="bootstrap")
```
...
ipcwqrPIC::dcrq(L=L,R=R,T=T,delta=delta,x=x,tau=tau,wttype="param",var.estimation="IS")
ipcwqrPIC::dcrq(L=L,R=R,T=T,delta=delta,x=x,tau=tau,wttype="param",estimation = "dr",var.estimation="IS")
ipcwqrPIC::dcrq(L=L,R=R,T=T,delta=delta,x=x,tau=tau,wttype = "Beran",hlimit = 0.9,var.estimation="IS")
ipcwqrPIC::dcrq(L=L,R=R,T=T,delta=delta,x=x,tau=tau,wttype = "Ishwaran",var.estimation="IS")
```

We can choose one of two weighting types, the parametric KM estimating method, Beran's nonparametric KM estimating method, and Ishwaran's nonparametric survival random forests estimating method which can be accessible by using wttype="param", wttype="Beran" and wttype="Ishwaran", respectively.
...
T = 1+x1+x2 + vi +(rnorm(n) - qnorm(tau))
...
ipcwqrPIC::dcrq(L=L,R=R,T=T,delta=delta,x=x,tau=tau,wttype="param",var.estimation="IS",id=NULL)
ipcwqrPIC::dcrq(L=L,R=R,T=T,delta=delta,x=x,tau=tau,wttype="param",var.estimation="IS",id=id,index = 1)
```
